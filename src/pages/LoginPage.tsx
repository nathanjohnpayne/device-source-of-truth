import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { ShieldX } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import LoadingSpinner from '../components/shared/LoadingSpinner';

const DOMAIN_DENIED_MESSAGE =
  'Your email domain is not authorized to access Device Source of Truth.';

function isDomainError(error: string): boolean {
  return error === DOMAIN_DENIED_MESSAGE || error.includes('not authorized');
}

export default function LoginPage() {
  const { user, loading, error, signIn } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    if (user) navigate('/', { replace: true });
  }, [user, navigate]);

  if (loading) return <LoadingSpinner className="min-h-screen" />;

  const domainDenied = error && isDomainError(error);

  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900">
      <div className="w-full max-w-md rounded-2xl bg-white p-10 shadow-2xl">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-indigo-600 text-xl font-bold text-white shadow-lg">
            D
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Device Source of Truth</h1>
          <p className="mt-1 text-sm text-gray-500">
            Disney Streaming — Partnerships & Devices
          </p>
        </div>

        {domainDenied ? (
          <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-6 text-center">
            <ShieldX className="mx-auto mb-3 h-10 w-10 text-red-500" />
            <h2 className="text-lg font-semibold text-red-800">Access Denied</h2>
            <p className="mt-2 text-sm text-red-700">
              Your email domain is not authorized to access this application.
            </p>
            <p className="mt-3 text-xs text-red-500">
              Please sign in using an approved Disney corporate account. Contact an
              administrator if you believe this is an error.
            </p>
            <button
              onClick={signIn}
              className="mt-5 inline-flex items-center gap-2 rounded-lg border border-red-300 bg-white px-4 py-2 text-sm font-medium text-red-700 shadow-sm transition-colors hover:bg-red-50"
            >
              Try a different account
            </button>
          </div>
        ) : (
          <>
            {error && (
              <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                {error}
              </div>
            )}

            <button
              onClick={signIn}
              className="flex w-full items-center justify-center gap-3 rounded-lg bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                  fill="#4285F4"
                />
                <path
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                  fill="#FBBC05"
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#EA4335"
                />
              </svg>
              Sign in with Google
            </button>

            <p className="mt-6 text-center text-xs text-gray-400">
              Restricted to Disney corporate accounts
            </p>
          </>
        )}
      </div>
    </div>
  );
}
