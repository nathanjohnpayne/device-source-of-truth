...
import { stripEmoji, normalizeCountries } from '../services/intakeParser.js';
...
function normalizeMigrationRegion(raw: string): IntakeRegion | null {
  const upper = raw.trim().toUpperCase();
  return REGION_MAP[upper] ?? null;
}

function normalizeMigrationCountry(raw: string): string[] {
  if (!raw.trim()) return [];
  const result = normalizeCountries(raw);
  if (!result.codes) return [];
  return result.codes.filter(c => !c.startsWith('UNKNOWN:'));
}
  ...
  try {
    ...
    for (const rawRow of parsed.data) {
      try {
        ...
        const region = normalizeMigrationRegion(row['region'] || '');
        const countries = normalizeMigrationCountry(row['country'] || '');
        ...
        await db.collection('devices').add({
          ...
          activeDeviceCount: parseInt(row['activeDeviceCount'] || '0') || 0,
          specCompleteness: 0,
          tierId: null,
          tierAssignedAt: null,
          region: region,
